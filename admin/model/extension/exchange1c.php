...
// -------------------- TEST --------------------
     ...
     * @return mixed
     * @throws Exception
     * Формування документа XML для вигрузки замовлень з сайту на 1с
     */
    public function queryOrders($params) {
        ...
        // $query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE `order_status_id` = " . $params['exchange_status'] . " ORDER BY `date_added` DESC  LIMIT 0,1");
        // } else {
        // $query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE `date_added` >= '" . $params['from_date'] . "' ORDER BY `date_added` DESC LIMIT 0,1");
        // }

        $lastnumber = isset($this->request->get['lastnumber']) ? (int)$this->request->get['lastnumber'] : 17887;

        if (isset($this->request->get['only_one'])) {
            $query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE (`order_id` > '" . $lastnumber . "' AND `order_status_id` != '0' AND `date_added` < '".date('Y-m-d H:i:s', time()-60)."') ORDER BY `order_id` LIMIT 0,1");
        } else {
            $query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE (`order_id` > '" . $lastnumber . "' AND `order_status_id` != '0' AND `date_added` < '".date('Y-m-d H:i:s', time()-60)."') ORDER BY `order_id` LIMIT 0,14");
        }
...
//            if ( ifAdmins() ) header("Content-Type:  text/xml; charset=UTF-8"); // ADD ADMIN XML

            foreach ($query->rows as $orders_data) {

                $order = $this->model_sale_order->getOrder($orders_data['order_id']);

                if ( (!$order['payment_lastname']) && (!$order['payment_firstname']) && (!(int)$order['total']) // пропускаем ошибочные заказы
                ...
                if($order['shipping_code'] == 'np.np') {
                    $order_simple 		= $this->db->query("SELECT osf.*, na.description AS area_name, ns.description AS settlement_name, nw.description AS warehouse_name 
						FROM `" . DB_PREFIX . "order_simple_fields` osf 
						LEFT JOIN `np_area` na ON (osf.shipping_np_area = na.ref)
						LEFT JOIN `np_settlement` ns ON (osf.shipping_np_settlement = ns.ref)
						LEFT JOIN `np_warehouse` nw ON (osf.shipping_np_warehouse = nw.ref)
						WHERE osf.order_id='". $order['order_id'] . "'");
//					aEcho($order_simple->row);

                    $order['shipping_city_id'] 	= $order_simple->row['shipping_np_settlement'];
                    $np_otdelenie 				= $order_simple->row['warehouse_name'];
                    $order['shipping_city'] 	= $order_simple->row['settlement_name'];
                    $order['shipping_address_1']= $order_simple->row['warehouse_name'];
                    $order['shipping_postcode']	= $order_simple->row['area_name'];

                    // need to be filled
                    $phone_np = $order_simple->row['phone_np'];
                    $fio_np = $order_simple->row['fio_np'];
                    $sum_np = (int) $order_simple->row['sum_np'];
                    $no_callback = (int)$order_simple->row['no_callback'];
                }
                elseif($order['shipping_code'] == 'np_courier.np_courier') {
                    $order_simple = $this->db->query("
						SELECT
							osf.*,
							na.description AS area_name,
							ns.description AS settlement_name
						FROM 
							`" . DB_PREFIX . "order_simple_fields` osf 
							LEFT JOIN `np_area` na ON (osf.shipping_np_area = na.ref)
							LEFT JOIN `np_settlement` ns ON (osf.shipping_np_settlement = ns.ref)
						WHERE 
							osf.order_id='". $order['order_id'] . "'");

                    $order['shipping_city_id'] 	= $order_simple->row['shipping_np_courier_settlement'];
                    $np_otdelenie 				= 'Доставка курьером';
                    $order['shipping_city'] 	= $order_simple->row['settlement_name'];
                    $order['shipping_address_1']= $order_simple->row['shipping_np_courier_street'] . ' ' . $order_simple->row['shipping_np_courier_building'] . ' ' . $order_simple->row['shipping_np_courier_appartment'];
                    $order['shipping_postcode']	= $order_simple->row['area_name'];
                    $np_street = $order_simple->row['shipping_np_courier_street'];
                    $np_building = $order_simple->row['shipping_np_courier_building'];
                    $np_appartment = $order_simple->row['shipping_np_courier_appartment'];
                    $phone_np = $order_simple->row['phone_np'];
                    $fio_np = $order_simple->row['fio_np'];
                    $sum_np = (int) $order_simple->row['sum_np'];
                    $no_callback = (int)$order_simple->row['no_callback'];
                }
                elseif($order['shipping_code'] == 'ukrpochta.ukrpochta') {
                    $order_simple = $this->db->query("
						SELECT
							*
						FROM 
							`" . DB_PREFIX . "order_simple_fields` 
						WHERE 
                    ...
                    $fio_np = $order_simple->row['fio_np'];
                    $sum_np = (int)$order_simple->row['sum_np'];
                    $order['shipping_postcode'] = $order_simple->row['shipping_ukrpochta_postcode'];
                    $no_callback = (int)$order_simple->row['no_callback'];
                }
                elseif($order['shipping_code'] == 'test_shipping.test_shipping') {
                    $order_simple = $this->db->query("
						SELECT
							*
						FROM 
                    ...
                    $fio_np = $order_simple->row['fio_np'];
                    $sum_np = (int)$order_simple->row['sum_np'];
                    $order['shipping_postcode'] = $order_simple->row['shipping_ukrpochta_postcode_'];
                    $no_callback = (int)$order_simple->row['no_callback'];
                }
                elseif($order['shipping_code'] == 'test_shipping2.test_shipping2') {
                    $order_simple 		= $this->db->query("SELECT osf.*, na.description AS area_name, ns.description AS settlement_name, nw.description AS warehouse_name 
						FROM `" . DB_PREFIX . "order_simple_fields` osf 
						LEFT JOIN `np_area` na ON (osf.shipping_np_test_area = na.ref)
						LEFT JOIN `np_settlement` ns ON (osf.shipping_np_test_settlement = ns.ref)
						LEFT JOIN `np_warehouse` nw ON (osf.shipping_np_test_warehouse = nw.ref)
						WHERE osf.order_id='". $order['order_id'] . "'");

                    $order['shipping_city_id'] 	= $order_simple->row['shipping_np_test_settlement'];
                    $np_otdelenie 				= $order_simple->row['warehouse_name'];
                    $order['shipping_city'] 	= $order_simple->row['settlement_name'];
                    $order['shipping_address_1']= $order_simple->row['warehouse_name'];
                    $order['shipping_postcode']	= $order_simple->row['area_name'];
                ...
                $commission = 0;
                if ( (isset($order['liqpay'])) && (!$order['liqpay'] == 0) ) {
                    $order_commission = $this->db->query("SELECT commission FROM `" . DB_PREFIX . "order_liqpay` WHERE order_id='". $order['order_id'] . "'");
                    if ($order_commission->num_rows) { $commission = $order_commission->rows[0]['commission']; }
                }
                ...
                $order['comment'] = iconv('utf-8', 'windows-1251//IGNORE', $order['comment']);
                $order['comment'] = iconv('windows-1251', 'utf-8//IGNORE', $order['comment']);

                // $order['telephone'] = iconv('utf-8', 'windows-1251//IGNORE', $order['telephone']);
                // $order['telephone'] = iconv('windows-1251', 'utf-8//IGNORE', $order['telephone']);
                $client_name = ( !empty($order['payment_lastname']) || !empty($order['payment_firstname']) ) ? ($order['payment_lastname'] . ' ' . $order['payment_firstname']) : ('В 1 клик/дроп. ' . trim($order['telephone'],'+ '));

                $document['Документ' . $document_counter] = array(
                    'Ид'          => $order['order_id']
                ,'Номер'       => $order['order_id']
                ,'НомерРеал'   => $order['order_id']
                ,'Дата'        => $date
                ,'Время'       => $time
                ...
                ,'Роль'        => 'Продавец'
                ,'Сумма'       => $order['total']
                ,'Комментарий' => 	'AGER - '.(($order['payment_lastname']) ?  $order['payment_lastname'] :  $order['lastname'] ) . ' ' .
                        (($order['payment_firstname']) ? $order['payment_firstname'] : $order['firstname']) .
                        '. тел: '.$order['telephone'].', '.' e-mail:'.$order['email'].'. Адрес доставки: '.$order['shipping_city'].', '. htmlspecialchars_decode($order['comment'])
//                ,'Не_перезванивать' => $no_callback
//                ,'Город_НП'	   		=> $order['shipping_city_id']
//                ,'Отделение_НП'	  	=> $np_otdelenie
//                ,'ОнлайнОплата' 	=> isset($order['liqpay']) ? $order['liqpay'] : ''
//                ,'ОнлайнКомиссия'	=> $commission
//                ,'СпособОплаты' 	=> $order['payment_code']
//                ,'КодДоставки' 		=> $order['shipping_code']
//                ,'Телефон_НП' 		=> empty($phone_np) ? $order['telephone'] : $phone_np
//                ,'ФИО_НП' 			=> empty($fio_np) ? $client_name : $fio_np
//                ,'Сумма_НП' 		=> $sum_np
//                ,'Сумма_Заказа_НП' 	=> ((isset($order['liqpay']))&&($order['liqpay']!=0)) ? 0 : $order['total']
//                ,'Улица_НП' 		=> $np_street
//                ,'Дом_НП' 			=> $np_building
//                ,'Квартира_НП' 		=> $np_appartment
//                ,'Индекс' 			=> $order['shipping_postcode']
                );
                ...
                ,'ИдСайта'                 => $order['customer_id']
                ,'ИдГрупп'		=> $order['customer_group_id']
                ...
                ,'Адрес' => array(
                        'Представление'	=> $order['shipping_address_1'].', '.$order['shipping_city'].', '.$order['shipping_postcode'].', '.$order['shipping_zone'].', '.$order['shipping_country']
                    )
                , 'Телефон' => trim($order['telephone'],'+ ')
                , 'Почта'   => $order['email']
                ,'Контакты' => array(
                        'Контакт1' => array(
                            'Тип' => 'ТелефонРабочий'
                ...
                foreach ($products as $product) {
                    ...
                    $new_id = $this->model_sale_order->getFullGuidProductByOrderProductId($product['order_product_id']);
                    if (!$new_id) {
                        $new_id = $this->get1CProductIdByProductId($product['product_id']);
                    }
                    ...
                    $product_summ = $product_summ + $product['total'];
                ...
                if($product_summ<>0) {$document['Документ' . $document_counter]['Скидка'] = round(($product_summ-$order['total'])*100/$product_summ,5);}
                $reward = $product_summ-$order['total'];
                if ($reward !==0 ) {
                    $document['Документ' . $document_counter]['СуммаСкидкиВБонусах'] = $reward;
                }
                $document_counter++;
            }
        }
        ...
        $xml = $this->array_to_xml($document, $SimpleXMLElement );
        $current = $xml->asXML();
        //file_put_contents(DIR_DOWNLOAD . 'orders-' . date('Y-m-d', time()) . '.xml', $current);
        return $current;
    }
...
//	} // queryOrders()
	 ...
	 * Обновляет документ
     * БЕРЕ УЧАСТЬ В ОБМІНІ ТОВАРАМИ!!!
     * оновлює замовлення в сайті з 1с незрозуміло для чого оновлює товар також в замовлені
	 */
...
//	} // updateDocument()
	 ...
	 * Разбор документа
	 * БЕРЕ УЧАСТЬ В ОБМІНІ ТОВАРАМИ!!!
     * отримання замовлення з 1с розбираємо його
	 */
...
//	} // parseDocument()
	...
	public function importFile($importFile, $type) {
		...
		// ORDERS.XML   тут приходить файл з 1с з ордером
//		if ($xml->Документ) {
