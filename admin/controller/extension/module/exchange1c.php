...
//			var_dump("<pre>"); var_dump($this->config->get('exchange1c_'.$param)); var_dump("</pre>");
	...
	private function htmlButton($name, $param) {
		...
		if (!empty($param['ver'])) {
			...
			//$onclick = ' onclick="$(\'#form-exchange1c\').attr(\'action\',\'' . $this->url->link('extension/module/exchange1c/update', 'user_token=' . $this->session->data['user_token'], 'SSL') . '&version=' . $param['ver'] . '\').submit()"';
		}
	...
	public function index($refresh = false) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/exchange1c', 'user_token=' . $this->session->data['user_token'], 'SSL'),
			'separator' => ' :: '
		);
		$data['user_token'] = $this->session->data['user_token'];
		$data['refresh'] = $this->url->link('extension/module/exchange1c/refresh', 'user_token=' . $this->session->data['user_token'], 'SSL');
		$data['action'] = $this->url->link('extension/module/exchange1c', 'user_token=' . $this->session->data['user_token'], 'SSL');
...
//		);
		...
		// изменение движка таблиц 

		

		$this->db->query("ALTER TABLE `oc_product` ENGINE = INNODB");
		$this->db->query("ALTER TABLE `oc_product` ALTER `date_available` DROP DEFAULT");

		$this->db->query("ALTER TABLE `oc_product_option` ENGINE = INNODB");
		$this->db->query("ALTER TABLE `oc_product_option_value` ENGINE = INNODB");
		
		$this->db->query("ALTER TABLE `oc_category` ENGINE = INNODB");
		$this->db->query("ALTER TABLE `oc_attribute` ENGINE = INNODB");
		$this->db->query("ALTER TABLE `oc_option` ENGINE = INNODB");
		$this->db->query("ALTER TABLE `oc_option_value` ENGINE = INNODB");
		$this->db->query("ALTER TABLE `oc_manufacturer` ENGINE = INNODB");
		$this->db->query("ALTER TABLE `oc_store` ENGINE = INNODB");
...
// // 				FOREIGN KEY (`warehouse_id`) 		REFERENCES `" . DB_PREFIX . "warehouse`(`warehouse_id`),
		...
		// $this->db->query("DROP TABLE IF EXISTS `oc_product_feature_value`");
		// $this->db->query("
		// CREATE TABLE IF NOT EXISTS `oc_product_feature_value` (
		//   `product_feature_id` int(11) NOT NULL,
		//   `product_option_id` int(11) NOT NULL,
		//   `product_id` int(11) NOT NULL,
		//   `product_option_value_id` int(11) NOT NULL,
		...
		//   CONSTRAINT `oc_product_feature_value_ibfk_1` FOREIGN KEY (`product_feature_id`) REFERENCES `oc_product_feature` (`product_feature_id`),
		//   CONSTRAINT `oc_product_feature_value_ibfk_2` FOREIGN KEY (`product_option_id`) REFERENCES `oc_product_option` (`product_option_id`),
		//   CONSTRAINT `oc_product_feature_value_ibfk_3` FOREIGN KEY (`product_id`) REFERENCES `oc_product` (`product_id`),
		//   CONSTRAINT `oc_product_feature_value_ibfk_4` FOREIGN KEY (`product_option_value_id`) REFERENCES `oc_product_option_value` (`product_option_value_id`)
		// ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='exchange1c'
		// ");	
...
//					}
	 ...
	 * БЕРЕ УЧАСТЬ В ОБМІНІ ТОВАРАМИ!!! - ГОЛОВНИЙ
	 *
...
//		if ( !ifAdmins() && !isset($this->request->cookie['key']) ) {
//			echo "Cookie fail\n";
//			return;
//		}
//
//		if ( !ifAdmins() && $this->request->cookie['key'] != md5($this->config->get('exchange1c_password')) ) {
//			echo "failure\n";
//			echo "Session error";
//			return;
//		}
		...
		$orders = iconv('utf-8', 'utf-8//IGNORE', $orders);

//		if ( ifAdmins() ) {
//			echo $orders;
//		} else {
//			echo iconv('utf-8', 'cp1251//IGNORE', $orders);
//		}
	}
